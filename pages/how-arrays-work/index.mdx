...
import ReadMemory from '@/components/how-arrays-work/ReadMemory'
import WriteMemory from '@/components/how-arrays-work/WriteMemory'
...
Before we get started, let's take a look at the memory APIs that are available to us. We're going to have access to a `Memory` object with four methods:
...
This memory object is designed to act just like the memory in your computer. If you've written code in C or C++ before, some of these methods might be familiar to you. If not, that's okay too — we'll go through each one in this section.
...
Ok, so our end goal here is to somehow make each element of the array the same size despite
